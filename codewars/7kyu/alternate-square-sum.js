...
function alternateSqSum(arr){
    let even = arr.filter((e, i) => i % 2 !== 0).reduce((acc, c) => acc + Math.pow(c, 2), 0)
    let odd = arr.filter((e, i) => i % 2 === 0).reduce((acc, c) => acc + c, 0)
    return even + odd
}

function alternateSqSum(arr){
    return arr.reduce((acc, c, index) => acc + ( index % 2 ? Math.pow(c, 2) : c), 0)
}